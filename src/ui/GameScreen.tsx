...
export function GameScreen(props: GameScreenProps) {
  ...
  const onWheel = (e: WheelEvent) => {
    ...
    if (e.deltaY > 0) {
      ...
      if (h.length >= 2) {
        wheelLast = now
        props.machine.rollback(h.length - 2)
      }
    }
  }
  ...
  return (
    <div class="game-screen" onWheel={onWheel}>
      <Stage
        story={props.story}
        scene={snap().scene}
      ...
      <Show when={overlay() === 'history'}>
        <HistoryOverlay
          ...
          onRollback={(i) => {
            props.machine.rollback(i)
            setOverlay(null)
          }}
          onReplayVoice={(file) => props.audio.playVoice(file)}
