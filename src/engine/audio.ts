...
export class AudioEngine {
  ...
  playSound(file: string, loop: boolean, fadein: number) {
    this.stopSound(0.2)
    const h = new Howl({ src: [audioBlobUrl(file) ?? assetUrl(file)], loop, volume: fadein > 0 ? 0 : this.volumes.sound, format: ['ogg'] })
    this.sound = h
  ...
  playVoice(file: string) {
    ...
    const h = new Howl({ src: [audioBlobUrl(file) ?? assetUrl(file)], volume: this.volumes.voice, format: ['ogg'], html5: true })
    h.once('end', () => {
      h.unload()
      this.clearVoice(h)
    })
